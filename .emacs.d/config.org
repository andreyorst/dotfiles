...
** Center View Mode
This is my  own mode, that I  like to use when writing  text in single
frame. It centers buffer contents, making it nice an clean.

First, we need a mode hook:
...
Next, let's create the minor mode:
...
Let's make =margin-size= a buffer local variable:
...
This is the main function that centers buffer contents:
