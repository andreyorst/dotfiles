...
way to switch between those without loosing ability to use such crords
...
"typewriter" layout with  numberline inverted, so to  access numbers I
needto use Shift key. =russian-computer= doesn't have this problem.
