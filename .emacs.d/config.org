...
can display all  relevant information in the mode  line.  Line numbers
are displayed by default, so  I just need to enable =column-number-mode=
feature.  Also  disable the  size  indication,  as  I find  it  pretty
useless.
