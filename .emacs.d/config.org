...
*** Spacemacs theme
    ...
    Also let's disable fringe background color, if theme changes it.
...
*** Org Mode
    ...
    #+BEGIN_SRC emacs-lisp
      (add-hook 'org-mode-hook (lambda()
                                 ...
                                 (yas-minor-mode)
                                 (yas-reload-all)
                                 (setq default-justification 'fullscreen)
                                 (auto-fill-mode)))
    #+END_SRC
...
*** Rust Mode
    These are  settings for  Rust buffers,  that will  enable language
    server and Flycheck.
