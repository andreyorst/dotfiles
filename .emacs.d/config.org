...
=require=  the =org-mode=  and change  the  default major  mode for  Emacs
startup.
...
I've decided to stop using line numbers at the side of the window, but
I still want to see which  line number I'm currently on. Luckily Emacs
can display all relevant information in  the mode line. I just need to
enable =column-number-mode= feature.
...
#+BEGIN_SRC emacs-lisp
  (defun my/org-tangle-on-config-save ()
    ...
    (when (string= buffer-file-name (file-truename "~/.emacs.d/config.org"))
      (org-babel-tangle)
      (load-file (file-truename "~/.emacs.d/config.el"))))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (defun my/eshell-toggle ()
    ...
    (interactive)
    (defvar name "*eshell-popup*")
    (cond ((get-buffer-window name)
...
[[*=my/update-inline-images=][=my/update-inline-images=]] function after executing code with babel.
...
Also let's disable fringe background  color and line numbers color, if
theme changes it.
...
** Diminish
Emacs, please  stop cluttering  my mode  line with  all those  modes I
...
#+BEGIN_SRC emacs-lisp
  (use-package vdiff
    ...
    (add-hook 'vdiff-mode-hook #'show-all))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package multiple-cursors
    ...
    :config (defhydra h-mc (global-map "C-c m")
              "multiple cursors"
              ("l" mc/edit-lines "edit lines" :exit t)
              ("e" mc/edit-ends-of-lines "edit ends of lines" :exit t)
              ("a" mc/edit-beginnings-of-lines "edit beginnings of lines" :exit t)
              ("A" mc/mark-all-in-region "mark all in region" :exit t)
              ("r" mc/mark-all-in-region-regexp "mark regexp in region" :exit t)
              ("n" mc/mark-next-like-this-word "mark next like this word")
              ("p" mc/mark-previous-like-this-word "mark previous like this word")
              ("d" mc/remove-duplicated-cursors "remove duplicated cursors" :exit t)))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package expand-region
    :config (defhydra h-er (global-map "C-c e")
              "expand region"
              ("e" er/expand-region "expand region" :exit t)
              ("p" er/mark-paragraph "mark paragraph")
              ("(" er/mark-inside-pairs "mark inside pairs")
              (")" er/mark-outside-pairs "mark outside pairs")
              ("q" er/mark-inside-quotes "mark inside quotes")
              ("Q" er/mark-outside-quotes "mark outside quotes")))
#+END_SRC
