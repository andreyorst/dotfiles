...
with C  code and Makefile rules.
...
#+BEGIN_SRC emacs-lisp
  (defun my/set-selected-frame-dark ()
    "set titlebar colorscheme to dark variant"
    (interactive)
    (if window-system
...
If  case I  use =window-system=  I want  this function  to run  during
initialization process.

#+BEGIN_SRC emacs-lisp
  (add-hook 'emacs-startup-hook 'my/set-selected-frame-dark :after)
#+END_SRC
...
I'm editing =emacs-lisp=  source blocks. But I find  it distracting to
see warnings like =;;; Code=, so let's disable it:
...
#+BEGIN_SRC emacs-lisp
  (use-package multiple-cursors
    ...
    :config (defhydra hydra-multiple-cursors (global-map "C-c m")
              "multiple cursors"
...
#+BEGIN_SRC emacs-lisp
  (use-package expand-region
    :config (defhydra hydra-expand-region (global-map "C-c e")
              "expand region"
