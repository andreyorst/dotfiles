...
define-command -hidden plug -params 1..2 %{
	%sh{
		if [[ -d $(eval echo $kak_opt_plug_install_dir) ]]; then
			if [[ -d $(eval echo $kak_opt_plug_install_dir/$(basename $1)) ]]; then
				for file in $(find -L $(eval echo $kak_opt_plug_install_dir/$(basename $1)) -type f -name '*.kak'); do
					echo source "$file"
				done
			fi
		fi
...
define-command plug-install -docstring 'Install all uninstalled plugins' %{
	...
	nop %sh{ (
		...
		for plugin in $kak_opt_plug_plugins; do
			if [[ ! -d $(eval echo $kak_opt_plug_install_dir/$(basename $plugin)) ]]; then
				(cd $(eval echo $kak_opt_plug_install_dir); git clone https://github.com/$plugin 2>&1 ) &
			fi
		done
		wait
...
define-command plug-update -docstring 'Update all installed plugins' %{
	...
	nop %sh{ (
		...
		for plugin in $kak_opt_plug_plugins; do
			(cd $(eval echo $kak_opt_plug_install_dir/$(basename $plugin)) && git pull 2>&1) &
		done
		wait
	...
	nop %sh{ (
		...
		for instlled_plugin in $(echo $(eval echo $kak_opt_plug_install_dir)/*); do
			...
			for enabled_plugin in $kak_opt_plug_plugins; do
				[[ $(basename $instlled_plugin) == $(basename $enabled_plugin) ]] && { skip=1; break; }
			done
			[[ -n $skip ]] || plugins_to_remove+=("$instlled_plugin")
		done
