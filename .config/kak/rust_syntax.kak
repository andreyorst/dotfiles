...
hook -group rust-highlight global WinSetOption filetype=rust %{
    ...
    hook -once -always window WinSetOption filetype=.* %{ remove-highlighter window/rust_syntax }
}
...
add-highlighter shared/rust_syntax/code/macro_attributes     regex (?:#!?\[.*?\]) 0:meta
add-highlighter shared/rust_syntax/code/namespace            regex [a-zA-Z](\w+)?(\h+)?(?=::) 0:module
add-highlighter shared/rust_syntax/code/macros               regex \b[A-z0-9_]+! 0:meta
